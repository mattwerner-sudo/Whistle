...
type SignalType = 'all' | 'new_hire' | 'departure' | 'tech_drop' | 'tech_add' | 'warm_path';
...
export default function SignalsPage() {
  ...
  const coreTypes = ['new_hire', 'departure', 'tech_drop', 'tech_add', 'warm_path'];
  const signals = (signalsQuery.data?.signals || []).filter(s => coreTypes.includes(s.type));
  ...
  const signalCounts = {
    ...
    warm_path: signals.filter(s => s.type === 'warm_path').length,
  };
